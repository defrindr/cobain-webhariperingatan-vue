	      ...
	      <div class="col-md-4 col-sm-12 col-xs-12" v-for="ev in events" :key="ev.date">
	        <article>
	          <div class="article__head">
	            ...
	            <button onclick="show(event)"> > </button>
	          </div>
...
export default {
	...
	methods: {
		...
		checkBulan() {
			if ( 
				( this.currentMonthInNumber-1 <= -2 ) ||
				( this.currentMonthInNumber+1 >= 13 )
				) {
		...
		bulanLalu() {
			...
			if ( this.checkBulan() ) {
				...
				this.events = this.findData(
					this.currentMonth.toLowerCase() )
			} else {
		...
		getNextEventInfo(){
			let evtempToNextEv = this.s.filter(data => {
				...
				let strtotime1 = new Date( Date.UTC(
								oldDate[2], 
								this.month.indexOf(oldDate[1]), 
								oldDate[0],
								) );
	...
	mounted() {
		...
		this.now = date.getDate() + " " + this.month[ this.currentMonthInNumber ] + " " + date.getFullYear();
